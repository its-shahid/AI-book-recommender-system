...
.info-label { font-weight: 700; color: #c0692a; font-size: 0.78rem;
...
# ─────────────────────────────────────────────
# Gemini API — Free Explanation
# ─────────────────────────────────────────────
def get_explanation(selected_book, recommended_books):
    ...
    prompt = (
        ...
        f"In 3-4 sentences, explain what themes, style, or audience these books "
        f"share with the original. Be warm, specific, and helpful. "
        f"Only talk about books, nothing else."
    )
    try:
        ...
        rec_list = ", ".join(recommended_books[:5])
        return (
            f"Because you liked <strong>{selected_book}</strong>, our recommendation engine analysed "
            f"the rating patterns of thousands of readers and found that users who enjoyed this book "
            f"also highly rated: <strong>{rec_list}</strong>. "
            f"These books share similar themes, writing style, or target audience. "
            f"Powered by Collaborative Filtering using K-Nearest Neighbors algorithm."
        )
...
<div class="hero">
  ...
  <p>Discover your next favourite read — powered by Collaborative Filtering</p>
...
if show:
    ...
    if status == "OUT_OF_DOMAIN" or not books_list:
        st.error("Could not find recommendations. Please try another book.")
        st.stop()
    ...
    with st.spinner("Analysing why these books match..."):
        explanation = get_explanation(selected_book, display_books)

    st.markdown(f"""
    <div class="info-box">
      <div class="info-label">&#128161; Why these books match</div>
      {explanation}
    </div>
    """, unsafe_allow_html=True)

    st.info(
        "Domain notice: This system only recommends books. "
